...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validation
    if (empty($first_name) || empty($last_name) || empty($email) || empty($contact_number) || empty($password) || empty($barangay) || empty($id_type)) {
        ...
        $error = "Invalid contact number. Must be 11 digits starting with 09.";
        ...
        if (!$error) {
            ...
            if ($check_stmt->num_rows > 0) {
                ...
                $check_contact_stmt = $conn->prepare("SELECT id FROM account WHERE contact_number = ?");
                $check_contact_stmt->bind_param("s", $contact_number);
                ...
                if ($check_contact_stmt->num_rows > 0) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO account (first_name, middle_name, last_name, email, contact_number, password, barangay, id_type, resident_type, file_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param("ssssssssss", $first_name, $middle_name, $last_name, $email, $contact_number, $password, $barangay, $id_type, $resident_type, $file_path);
...
<div>
    <div class="input-group">
        ...
        <input class="input-box" type="text" name="contact_number" placeholder="Contact Number" required value="<?php echo isset($_POST['contact_number']) ? htmlspecialchars($_POST['contact_number']) : ''; ?>">
    </div>
</div>
