...
<head>
  ...
  <style>
    * {
    ...
    .header-actions .btn.disable {
  background: #dc2626;
}

.header-actions .btn.enable {
  background: #16a34a;
}
...
<body>
  ...
  <button id="toggleAccountBtn" class="btn" style="display: none;">
    <i id="toggleIcon" class="fas fa-ban"></i>
    <span id="toggleText">Disable Account</span>
  </button>
...
async function toggleAccountStatus() {
  ...
  const confirmMsg = isDisabled 
    ...
    : 'Are you sure you want to disable this account? The user will not be able to log in until re-enabled.';
  
  if (!confirm(confirmMsg)) return;
  
  btn.disabled = true;
  text.textContent = isDisabled ? 'Enabling...' : 'Disabling...';
  
  try {
    const response = await fetch('api_toggle_account.php', {
      method: 'POST',
    ...
    if (data.success) {
      ...
      btn.dataset.disabled = data.is_disabled ? 'true' : 'false';
      if (data.is_disabled) {
      ...
      // Update badges
      updateAccountBadges(data.is_disabled);
...
function updateAccountBadges(isDisabled) {
  const badgesContainer = document.getElementById('userBadges');
  
  // Remove existing disabled badge if any
  const existingBadge = badgesContainer.querySelector('.badge-disabled');
  if (existingBadge) {
    existingBadge.remove();
  }
  
  // Add disabled badge if account is disabled
  if (isDisabled) {
    const disabledBadge = document.createElement('span');
    disabledBadge.className = 'badge badge-disabled';
    disabledBadge.innerHTML = '<i class="fas fa-ban"></i> Account Disabled';
    badgesContainer.insertBefore(disabledBadge, badgesContainer.firstChild);
  }
}

// Update displayUserProfile function to initialize toggle button properly
function displayUserProfile(user) {
  // Hide loading, show profile
  document.getElementById('loadingState').style.display = 'none';
  document.getElementById('profileContainer').classList.add('show');
  ...
  // Add disabled badge first if account is disabled
  if (user.isDisabled) {
    badgesHtml += `
  ...
  toggleBtn.dataset.disabled = user.isDisabled ? 'true' : 'false';
  ...
  toggleBtn.onclick = toggleAccountStatus;
