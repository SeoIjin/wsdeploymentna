...
while ($row = $result->fetch_assoc()) {
    ...
    // Keep status values consistent - use proper casing for display
    // but ensure the value matches what JavaScript expects
    switch ($status) {
        case 'PENDING':
            $row['status'] = 'Pending';
            break;
        ...
        case 'COMPLETED':
            ...
            break;
        case 'REJECTED':
            // CRITICAL FIX: Keep this as 'Rejected' to match JS toLowerCase() comparison
            $row['status'] = 'Rejected';
            break;
        default:
