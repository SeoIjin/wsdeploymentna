...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // MODIFIED: Added is_disabled to the SELECT query
    $stmt = $conn->prepare("SELECT id, password, usertype, first_name, last_name, account_status, is_disabled FROM account WHERE email = ?");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $stmt->store_result();
    // MODIFIED: Added is_disabled to bind_result
    $stmt->bind_result($id, $stored_password, $usertype, $first_name, $last_name, $account_status, $is_disabled);
    $stmt->fetch();
    ...
    if ($stmt->num_rows > 0) {
        ...
        if ($password_correct) {
            // NEW: Check if account is disabled FIRST
            if ($is_disabled == 1) {
                $error = "Your account has been disabled by the administrator. Please contact support for assistance.";
            } else {
                // Check if account is approved (skip check for admin accounts)
                $email_domain = '';
                ...
                $is_admin = ($email_domain === 'gov.ph' || $usertype === 'admin');
                
                if (!$is_admin && $account_status !== 'approved') {
                    $error = "Your account is pending approval. Please wait for admin verification before signing in.";
                    
                } else {
                    // Account is approved or is admin - proceed with login
                    $_SESSION['user_id'] = $id;
    ...
    <div class="container">
        <div class="left-panel">
            ...
            <div class="welcome-text">
                Welcome to BCDRS (Barangay Citizen Document Request System)<br>Your friendly assistant is here to help!
            </div>
        </div>
        <div class="right-panel">
            ...
            <div class="form-container">
                ...
                <div class="form-content">
                    ...
                    <!-- Guest Portal Button -->
                    <button class="guest-btn" type="button" onclick="location.href='index.php'">
